...
bool TriLidar::begin(HardwareSerial &serialobj, uint32_t baudrate) {
  if (isOpen()) {
    end();
  }
...
bool TriLidar::isOpen(void) {
  return _bined_serialdev ? true : false;
}

void TriLidar::setIntensity(int i) {
  intensity = i;
  //设置强度位数时重新分配内存
  if (packageBuffer)
  {
...
void TriLidar::setSingleChannel(bool yes) {
  singleChannel = yes;
  if (yes)
  {
    //单通雷达需要通过dtr控制启动
...
//设置电机速度
void TriLidar::setMotorSpeed(float vol) {
  uint8_t PWM = (uint8_t)(51 * vol);
  analogWrite(YDLIDAR_MOTOR_SCTP, PWM);
}

// ask the YDLIDAR for its device health
result_t TriLidar::getHealth(device_health &health, uint32_t timeout) {
  result_t  ans;
  uint8_t  recvPos = 0;
...
result_t TriLidar::getDeviceInfo(device_info &info, uint32_t timeout) {
  result_t  ans;
  uint8_t  recvPos = 0;
  uint32_t currentTs = millis();
  uint32_t remainingtime;
...
result_t TriLidar::startScan(bool force, uint32_t timeout) 
{
  result_t ans;
...
// wait scan data
result_t TriLidar::waitScanDot(uint32_t timeout) 
{
  if (!isOpen())
    return RESULT_FAIL;
...
//  while (i<10)
//  {
//    if (_bined_serialdev->available()>0)
//    {
//      i ++;
//      uint8_t c = _bined_serialdev->read();
//      if (lastC == 0xAA && c == 0x55)
//        printf("\n");
//      printf("%02X", c);
//
//      lastC = c;
...
//send data to serial
result_t TriLidar::sendCommand(uint8_t cmd, const void *payload, size_t payloadsize) {
...
result_t TriLidar::waitResponseHeader(
